<template>
  <div>
    <v-card tile outlined>
      <v-card-title>Índice legislativo - {{ pageData.title }}</v-card-title>
      <v-card-subtitle>
        Procure leis artigos e decretos utilizando nosso índice legislativo
      </v-card-subtitle>
    </v-card>
    <div>
      <template v-if="pageType === 'subPage'">
        <template v-if="grid">
          <v-row>
            <v-col
              v-for="item in pageData.children"
              :key="item.id"
              cols="12"
              sm="12"
              md="2"
              lg="2"
            >
              <v-card
                class="mx-auto mt-16"
                tile
                outlined
                :to="currentSlug + '/' + item.slug"
              >
                <v-card-title>
                  {{ item.title }}
                </v-card-title>
              </v-card>
            </v-col>
          </v-row>
        </template>
        <template v-else>
          <v-card max-width="600" class="mx-auto mt-16" tile outlined>
            <v-list subheader>
              <v-list-item
                v-for="item in pageData.children"
                :key="item.id"
                :to="currentSlug + '/' + item.slug"
              >
                <v-list-item-avatar tile>
                  <v-icon class="grey lighten-1" dark>
                    mdi-book-open-page-variant
                  </v-icon>
                </v-list-item-avatar>

                <v-list-item-content>
                  <v-list-item-title>{{ item.title }}</v-list-item-title>
                </v-list-item-content>

                <v-list-item-action>
                  <v-btn icon>
                    <v-icon color="grey lighten-1">
                      mdi-arrow-right-drop-circle-outline
                    </v-icon>
                  </v-btn>
                </v-list-item-action>
              </v-list-item>
            </v-list>
          </v-card>
        </template>
      </template>
      <template v-if="pageType === 'result'">
        <template v-if="grid">
          <v-row>
            <v-col
              v-for="item in pageData"
              :key="item.id"
              cols="12"
              sm="12"
              md="2"
              lg="2"
            >
              <v-card
                class="mx-auto mt-16"
                tile
                outlined
                :to="'/legislacao/' + item.slug"
              >
                <v-title>
                  {{ item.title }}
                </v-title>
              </v-card>
            </v-col>
          </v-row>
        </template>
        <template v-else>
          <v-card max-width="600" class="mx-auto mt-16" tile outlined>
            <v-list subheader>
              <v-list-item
                v-for="item in pageData"
                :key="item.id"
                :to="'/legislacao/' + item.slug"
              >
                <v-list-item-avatar tile>
                  <v-icon class="grey lighten-1" dark>
                    mdi-book-open-page-variant
                  </v-icon>
                </v-list-item-avatar>

                <v-list-item-content>
                  <v-list-item-title>{{ item.title }}</v-list-item-title>
                </v-list-item-content>

                <v-list-item-action>
                  <v-btn icon>
                    <v-icon color="grey lighten-1">
                      mdi-arrow-right-drop-circle-outline
                    </v-icon>
                  </v-btn>
                </v-list-item-action>
              </v-list-item>
            </v-list>
          </v-card>
        </template>
      </template>
    </div>
  </div>
</template>
<script>
export default {
  watchQuery: ['q'],
  async asyncData({ query, $axios, params }) {
    const currentRoute = params.pathMatch.split('/').slice(-1)[0]
    try {
      const { data } = await $axios.get(`/category-pages/${currentRoute}/sub`)

      // const pageResult = data.type === 'result' ? data : data.page

      return {
        pageData: data.page,
        pageType: data.type,
        currentSlug: currentRoute,
        grid: data.page.grid,
      }
    } catch (error) {
      console.log(params)
    }
  },
  head() {
    return {
      title: 'Índice legislativo',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content:
            'Procure leis, decretos e jurisprudências utilizando o índice legislativo Brasileis',
        },
      ],
    }
  },
}
</script>
<style>
.resultsInfo {
  color: #8e8e8e;
  font-size: 14px;
}
.p-bottom {
  margin-bottom: 25px;
}
em {
  color: #b82f00 !important;
  font-weight: 600 !important;
}
h1 {
  text-align: center;
}
</style>
