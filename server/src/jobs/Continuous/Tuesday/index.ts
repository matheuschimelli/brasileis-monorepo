...
export default {
  ...
  async handle(job: Job) {
    console.log(`Checking Tuesday Jobs ${dayjs().format('HH:mm')}`)
    try {
      ...
      const crawlers = await crawlerRepo.createQueryBuilder('crawler')
        ...
        .where('updateTime.tuesday= :update', { update: true })
        .andWhere('updateTime.updateTuesdayTime= :updateTuesdayTime', { updateTuesdayTime: dayjs().format('HH:mm') })
        .getMany()
      ...
      return Promise.resolve({ done: true, message: 'All pending jobs for Tuesday were queued ' })
    } catch (error: any) {
      console.log(error)
