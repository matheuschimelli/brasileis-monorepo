...
export default {
  ...
  async handle (job: Job) {
    console.log(`Checking Sunday Jobs ${dayjs().format('HH:mm')}`)
    ...
    try {
      ...
      const crawlers = await crawlerRepo.createQueryBuilder('crawler')
        ...
        .where('updateTime.sunday= :update', { update: true })
        .andWhere('updateTime.updateSundayTime= :updateSundayTime', { updateSundayTime: dayjs().format('HH:mm') })
        .getMany()
      ...
      return Promise.resolve({ done: true, message: 'All pending jobs for Sunday were queued ' })
    } catch (error) {
      console.log(error)
