...
export default {
  ...
  async handle(job: Job) {
    console.log(`Checking Thursday Jobs ${dayjs().format('HH:mm')}`)
    ...
    try {
      ...
      const crawlers = await crawlerRepo.createQueryBuilder('crawler')
        ...
        .where('updateTime.thursday= :update', { update: true })
        .andWhere('updateTime.updateThursdayTime= :updateThursdayTime', { updateThursdayTime: dayjs().format('HH:mm') })
        .getMany()
      ...
      return Promise.resolve({ done: true, message: 'All pending jobs for Thurday were queued ' })
    } catch (error) {
      console.log(error)
