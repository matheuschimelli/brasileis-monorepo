...
export default {
  ...
  async handle(job: Job) {
    console.log(`Checking Wednesday Jobs ${dayjs().format('HH:mm')}`)
    ...
    try {
      ...
      const crawlers = await crawlerRepo.createQueryBuilder('crawler')
        ...
        .where('updateTime.wednesday= :update', { update: true })
        .andWhere('updateTime.updateWednesdayTime= :updateWednesdayTime', { updateWednesdayTime: dayjs().format('HH:mm') })
        .getMany()
      ...
      return Promise.resolve({ done: true, message: 'All pending jobs for Wednesday were queued ' })
    } catch (error: any) {
      console.log(error)
